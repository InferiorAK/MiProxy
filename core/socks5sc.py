...
def file(key):
    ...
    if os.path.exists(args.output):
        ...
        outfile += args.output
    cnt = 0
    for line in proxies_socks5:
        cnt += 1
        serial = str(cnt)+". "
        proxy = line.split("\n")[0]
        try:
            response_socks5 = requests.get(url=url2_api, proxies={"http":"socks5://"+proxy, "https":"socks5://"+proxy}, timeout=args.timeout)
            if response_socks5.status_code == 200:
                success = open(outfile,"a").write(proxy+f" | {key}: "+str(response_socks5.json()[key])+"\n")
                print(f"{g}{serial}{proxy} success | {key}: "+str(response_socks5.json()[key]))
            else:
                print(f"{y}{serial}{proxy} "+str(response_socks5.status_code))
        except KeyError:
            try:
    ...
    for line in proxies_socks5:
        ...
        if (args.url).startwith("http://") or (args.url).startwith("https://"):
            try:
                ...
                response_socks5 = requests.get(url=args.url, proxies={"http":"socks5://"+proxy, "https":"socks5://"+proxy}, timeout=args.timeout)
                if response_socks5.status_code == 200:
                ...
                if args.dC:
                    ...
                    for line in proxies_socks5:
                        ...
                        try:
                            response_socks5 = requests.get(url=url2_api, proxies={"http":"socks5://"+proxy, "https":"socks5://"+proxy}, timeout=args.timeout)
                            if response_socks5.status_code == 200:
                                success = open(outfile,"a").write(proxy+" | location: "+str(response_socks5.json()[key1])+","+str(response_socks5.json()[key2])+"\n")
                                print(f"{g}{serial}{proxy} success | location: "+str(response_socks5.json()[key1])+","+str(response_socks5.json()[key2]))
                            else:
                                print(f"{y}{serial}{proxy} "+str(response_socks5.status_code))
                        except KeyError:
                            try:
                    ...
                    if args.file is None:
                        ...
                        for line in proxies_socks5:
                            ...
                            proxy = line.split("\n")[0]
                            try:
                                response_socks5 = requests.get(url=url_api, proxies={"http":"socks5://"+proxy, "https":"socks5://"+proxy}, timeout=args.timeout)
                                if response_socks5.status_code == 200:
                                    success = open(outfile,"a").write(proxy+"\n")
                                    print(f"{g}{serial}{proxy} success")
                                else:
